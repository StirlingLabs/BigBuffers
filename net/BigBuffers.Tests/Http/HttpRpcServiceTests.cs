...
{
  [SingleThreaded]
  public class HttpRpcServiceTests
  {
    static HttpRpcServiceTests()
    {
      ProfileOptimization.SetProfileRoot(Environment.CurrentDirectory);
      ProfileOptimization.StartProfile(nameof(HttpRpcServiceTests));
    }
    ...
    )
    {
      ...
      var svc = new HttpRpcServiceTestImpl("Test", ct, logger: logger);

      whb.Configure(ab => {
        ab.UseHttpRpcService(svc);
      });
    ...
    public sealed class HttpRpcServiceTestImpl : HttpRpcServiceServerBase
    {
      public HttpRpcServiceTestImpl(string name, CancellationToken ct, string? basePath = null,
        TextWriter? logger = null)
        : base(name, ct, basePath, logger) { }
      ...
      protected override Task Dispatch(string method, HttpContext ctx)
        => Dispatch(Enum.Parse<Method>(method), ctx);

      protected override async Task<ByteBuffer> DispatchUnary<TMethodEnum>(
        TMethodEnum method,
      ...
      protected override async Task<ByteBuffer> DispatchClientStreaming<TMethodEnum>(
        TMethodEnum method,
