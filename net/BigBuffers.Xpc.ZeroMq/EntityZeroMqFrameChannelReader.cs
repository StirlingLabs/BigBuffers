...
{
  [PublicAPI]
  public class EntityZeroMqFrameChannelReader<T> : ChannelReader<T> where T : struct, IBigBufferEntity
  {
    [Discardable]
    ...
    public EntityZeroMqFrameChannelReader(AsyncProducerConsumerCollection<ByteBuffer> collection, TextWriter? logger = null)
    {
      ...
      for (;;)
      {
        ...
        if (!_collection.IsCompleted)
        {
          ...
          catch (OperationCanceledException)
          {
            ...
            {
              _logger?.WriteLine(
                $"[{TimeStamp:F3}] {GetType().Name}<{typeof(T).Name}> T{Task.CurrentId}: waiting ({TimeSpent()}) was cancelled");
              return false;
            }
            _logger?.WriteLine(
              $"[{TimeStamp:F3}] {GetType().Name}<{typeof(T).Name}> T{Task.CurrentId}: waiting ({TimeSpent()}) was cancelled externally");
          }
