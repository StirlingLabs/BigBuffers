...
#define FLATBUFFERS_GEN_TYPES_POINTER(TD) \
  TD(STRING, "string", Offset<void>, long,    uint64,  StringOffset, uint64,  unused, ULong,   Offset<String>) \
  TD(VECTOR, "",       Offset<void>, long,    uint64,  VectorOffset, uint64,  unused, ULong,   Offset<UOffset>) \
  TD(STRUCT, "",       Offset<void>, long,    uint64,  ulong,        uint64,  unused, ULong,   Offset<UOffset>) \
  TD(UNION,  "",       Offset<void>, long,    uint64,  ulong,        uint64,  unused, ULong,   Offset<UOffset>)
#define FLATBUFFERS_GEN_TYPE_ARRAY(TD) \
  TD(ARRAY,  "",       uint64_t,     long,    uint64,  ulong,        uint64,  unused, ULong,   Offset<UOffset>)
// The fields are:
// - enum
...
struct EnumDef : public Definition {
  ...
  const EnumVal *Lookup(const std::string &enum_name) const {
    // check Value
    auto result = vals.Lookup(enum_name);
    if (result) return result;

    // check Enum.Value
    auto prefix = this->name + ".";
    if ( enum_name.rfind(prefix,0) == 0 ) {
      result = vals.Lookup(enum_name.substr(prefix.size()));
    }
    if (result) return result;

    // check Namespace.Enum.Value
    prefix = this->defined_namespace->GetFullyQualifiedName(prefix);
    if ( enum_name.rfind(prefix,0) == 0 ) {
      result = vals.Lookup(enum_name.substr(prefix.size()));
    }
    return result;
...
class Parser : public ParserState {
 ...
 public:
  ...
  mutable std::string error_;  // User readable error_ if Parse() == false
...
extern std::string MakeCamel(const std::string &in, bool firstUppercase = true);
...
extern bool GenerateJsonSchema(const Parser &parser, std::string *json, std::string &error);
